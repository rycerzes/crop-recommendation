...
export default function Home() {
  ...
  const fetchAllData = async (lat: number, lng: number) => {
    ...
    try {
      ...
      if (
        data.partialSuccess &&
        (!data.weather || !data.aqi || !data.forecast || !data.soil)
      ) {
        setError(
          "Some data could not be loaded, but showing available information"
        );
      }
  ...
  return (
    <main className="container mx-auto p-4">
      ...
      {error && (
        <Alert
          variant={error.includes("Some data") ? "default" : "destructive"}
          className="mt-4"
        >
          <AlertCircle className="h-4 w-4" />
          <AlertTitle>
            {error.includes("Some data") ? "Warning" : "Error"}
          </AlertTitle>
